...
- User's wording might have typos or informal phrasing of a KPI, product, section, or enum value, resolve it to the correct canonical spelling using the KPI context and KNOWN EXACT COLUMN VALUES given below.
- Fetch only the raw rows needed to answer the question - never compute comparisons, deviations, or percentages using CTEs/maths in SQL, only select statement, further calculation will be done in the next nodes.
...
4. Only use today's date for relative phrases ("today", "this month") with no history to inherit from.
  -4a. If the question mentions no date at all AND history resolves no date either, default to date range = current month (month of TODAY_DATE) with granularity = 'month' - never return unfiltered/all-history rows.
5. For a follow up qus give priority to the the same kpi_family as the previous question to check if sql can be generated by this kpi family, unless the current question belongs to a different KPI group.
6. For relative ranges like "last N days", resolve to literal dates using TODAY_DATE (e.g. "last 2 days" = TODAY_DATE-1 and TODAY_DATE-2 as Date literals) - never use current_date or INTERVAL in SQL.
...
def _build_user_prompt(state: AgentState) -> str:
    ...
    if state.get("intent") == "plot":
        parts.append(
            "\nPLOT MODE: this result will be charted, so it must contain "
            "multiple rows - a single row makes a chart pointless.\n"
            "First resolve the time scope (which month/year/range this question "
            "is about) exactly as you normally would, including from history if "
            "this is a follow-up (e.g. 'this month' from an earlier question).\n"
            "Then pick granularity as one level finer than that scope, so the "
            "chart has one point per sub-period within it:\n"
            "- Scope is a month -> granularity = 'day' (one point per day in that month).\n"
            "- Scope is a year -> granularity = 'month' (one point per month in that year).\n"
            "- Scope is an explicit multi-day date range -> granularity = 'day' across that range.\n"
            "- The user explicitly names a granularity (daily/monthly/shift-wise) -> use that instead, "
            "regardless of scope.\n"
            "- No date/scope resolvable from the question or history at all -> "
            "default to the current month at day granularity.\n"
            "Never copy a granularity directly from a history SQL entry without "
...
def generate_sql(state: AgentState) -> AgentState:
    ...
    state["current_kpi_family"] = result.kpi_family_selected.strip()

    logger.info("generate_sql: generated SQL | %s", state["generated_sql"])
    logger.info("current_kpi_family: Current Kpi Family  | %s", state["current_kpi_family"])

    history = state.get("query_history") or []
    history.append(QueryHistoryEntry(
        user_query=state["user_query"],
        generated_sql=state["generated_sql"],
        kpi_family=result.kpi_family_selected.strip(),
    ))
